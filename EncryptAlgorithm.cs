...
namespace Kuznecik_GOST
{
    public class EncryptAlgorithm
    {
        ...
        /// <summary>
        /// Коэффициенты линейного преобразования в шестнадцатиричной СС
        /// </summary>
        ...
        public string Decrypt(byte[] data)
        {
            ...
            for (int i = 8; i >= 0; i--)
            {
                ReverseLSX(ref temp, ref _keys[i], ref block);
        ...
        /// <summary>
        /// Рассчитываем раундовые ключи
        /// </summary>
        /// <param name="key">Массив байт ключа шифрования</param>
        public byte[][] GenerateRoundKey(byte[] key)
        {
            _keys = new byte[10][];
            for (int i = 0; i < 10; i++)
            {
            ...
            void C(ref byte[] c, int i)
            {
                Array.Clear(c, 0, _size);
                c[15] = (byte)i;
                L(ref c);
            }
        }
        ...
        private void R(ref byte[] data)
        {
            ...
            {
                data[i] = data[i - 1];
                x ^= _resultMultiplication[data[i - 1]][_lVector[i - 1]];
            }
            data[0] = x;
        }
        ...
        /// <summary>
        /// Преобразования ячейку Фейстеля
        /// </summary>
        /// <param name="k"></param>
        /// <param name="a1"></param>
        /// <param name="a0"></param>
        ...
        private void ReverseR(ref byte[] data)
        {
            ...
            {
                data[i] = data[i + 1];
                x ^= _resultMultiplication[data[i]][_lVector[i]];
            }
            data[15] = x;
        }
